...
At every turn, the model receives:
- All previous messages
- Plus the new user input

It processes the entire combined text from scratch — as if it were seeing it for the first time.

This means:
- **Cost** increases because more tokens are processed.
- **Response time** increases because more text has to be analyzed.
- **Risk of exceeding the context window** increases.

And very importantly:  
**The model does not have persistent memory** between chats or sessions.  
Everything it knows is only what you send **right now** in the current context.
:::
