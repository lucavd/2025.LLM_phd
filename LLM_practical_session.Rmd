...
call_lm_studio <- function(prompt, 
  ...
  tryCatch({
    ...
    # If raw_response is TRUE, return the complete response for debugging
    if (raw_response) {
    ...
    # Extract only the content from different possible response formats
...
> 5.  **Debugging Option:** The new `raw_response` parameter allows viewing the complete API response for troubleshooting
>
...
# First, let's check the raw response format
...
# Print raw response to see the format
cat("Raw API response:\n")
cat(raw_result)
cat("\n\n---\n\n")

# Now get just the content
...
cat("Extracted content only:\n")
cat(result)
```

> This two-part test serves multiple purposes:
>
> 1. **API Verification:** Confirms our local server is active and responding
> 2. **Response Format Inspection:** Shows the raw API response structure (helpful for debugging)
> 3. **Content Extraction:** Demonstrates how the function extracts just the content
> 4. **Medical Knowledge Testing:** Verifies the model can interpret basic laboratory data
>
> The second prompt requests interpretation of values consistent with microcytic anemia (low Hb, low MCV, high RDW), typically associated with iron deficiency. The `raw_response = TRUE` parameter is particularly useful when troubleshooting API integration issues.
